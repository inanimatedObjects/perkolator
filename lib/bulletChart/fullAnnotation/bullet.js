...
d3.bullet = function() {
  ...
  let orient = "left", // FIXME: needs in-line comment
  ...
  function bullet(g) {     // create accessor function for chart's <SVG><G> elements
    g.each(function(d, i) {  // create iterator function to access each <SVG><G> element in chart
      ...
      var x1 = d3.scale.linear()     // set d3 scaleLinear function to variable for DRY purposes
      ...
      var x0 = this.__chart__ || d3.scale.linear()  // set old scale to variable for DRY purposes. If none exist, set d3 scale placeholder
